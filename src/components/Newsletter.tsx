...
export default function Newsletter() {
  ...
  return (
    <section className="newsletter-section px-4 md:px-8 py-12 max-w-[1440px] mx-auto">
      <div className="newsletter-panel bg-coral/5 border border-coral/20 rounded-2xl sm:rounded-[2.5rem] p-6 sm:p-8 md:p-14 lg:p-20 flex flex-col lg:flex-row items-center justify-between gap-8 sm:gap-12 relative overflow-hidden shadow-sm">
        ...
        <div className="w-full max-w-md flex flex-col gap-4 sm:gap-6 relative z-10">
          <form 
            onSubmit={handleSubmit}
            className="flex bg-white border border-border rounded-full p-1.5 sm:p-2 shadow-xl focus-within:ring-4 focus-within:ring-coral/10 focus-within:border-coral transition-all"
          >
            <div className="flex-1 px-3 sm:px-4 py-2 flex items-center text-muted-foreground">
              <Mail className="mr-2 sm:mr-3 size-[18px] sm:size-[20px] text-coral/60" />
              <input 
                type="email"
                value={email}
                ...
                autoComplete="email"
                className="w-full bg-transparent outline-none text-dark text-sm sm:text-base font-medium placeholder:text-gray-300"
            ...
            <button
              ...
              className={cn(
                "font-bold px-5 sm:px-8 py-2.5 sm:py-3 rounded-full transition-all shadow-md whitespace-nowrap active:scale-95 text-sm sm:text-base",
                status === 'success' 
                  ? "bg-teal text-white" 
          ...
          <div className="flex items-center justify-center lg:justify-start gap-3 sm:gap-4">
            <span className="text-xs sm:text-sm font-bold text-muted-foreground mr-1 sm:mr-2 tracking-wide uppercase text-[10px]">Follow us:</span>
            {[
